    ...
    childRow: function (a, c, d) {
      ...
      return function (c, d, f) {
        if (d) b("div.dtr-modal-content").empty().append(f()); else {
          ...
          }, l = b('<div className="dtr-modal"/>').append(b('<div className="dtr-modal-display"/>').append(b('<div className="dtr-modal-content"/>').append(f())).append(b('<div className="dtr-modal-close">&times;</div>').click(function () { g() }))).append(b('<div className="dtr-modal-background"/>').click(function () { g() })).appendTo("body"); b(m).on("keyup.dtr", function (h) { 27 === h.keyCode && (h.stopPropagation(), g()) })
        } a && a.header && b("div.dtr-modal-content").prepend("<h2>" + a.header(c) + "</h2>")
      }
    }
  }; var A = {}; u.renderer =
  {
    listHiddenNodes: function () { return function (a, c, d) { var f = b('<ul data-dtr-index="' + c + '" className="dtr-details"/>'), g = !1; b.each(d, function (l, h) { h.hidden && (b("<li " + (h.className ? 'className="' + h.className + '"' : "") + ' data-dtr-index="' + h.columnIndex + '" data-dt-row="' + h.rowIndex + '" data-dt-column="' + h.columnIndex + '"><span className="dtr-title">' + h.title + "</span> </li>").append(b('<span className="dtr-data"/>').append(p(a, h.rowIndex, h.columnIndex))).appendTo(f), g = !0) }); return g ? f : !1 } }, listHidden: function () {
      return function (a,
        c, d) { return (a = b.map(d, function (f) { var g = f.className ? 'className="' + f.className + '"' : ""; return f.hidden ? "<li " + g + ' data-dtr-index="' + f.columnIndex + '" data-dt-row="' + f.rowIndex + '" data-dt-column="' + f.columnIndex + '"><span className="dtr-title">' + f.title + '</span> <span className="dtr-data">' + f.data + "</span></li>" : "" }).join("")) ? b('<ul data-dtr-index="' + c + '" className="dtr-details"/>').append(a) : !1 }
    }, tableAll: function (a) {
      a = b.extend({ tableClass: "" }, a); return function (c, d, f) {
        c = b.map(f, function (g) {
          return "<tr " + (g.className ?
            'className="' + g.className + '"' : "") + ' data-dt-row="' + g.rowIndex + '" data-dt-column="' + g.columnIndex + '"><td>' + g.title + ":</td> <td>" + g.data + "</td></tr>"
        }).join(""); return b('<table className="' + a.tableClass + ' dtr-details" width="100%"/>').append(c)
      }
    }
