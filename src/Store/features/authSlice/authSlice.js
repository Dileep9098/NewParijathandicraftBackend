...
//         } catch (error) {
//             return rejectWithValue(error.response?.data || 'Registered failed due to an unknown error');
...
export const registerUser = createAsyncThunk(
    "/register",
    async ({ email, cpassword, password }, { rejectWithValue }) => {
        try {
            // debugger
            const response = await axiosInstance.post(
                "/api/v1/register/",
...
export const loadUser = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
      try {
        ...
        return rejectWithValue(error.response?.data || 'Error loading user');
      }
    }
  );
